...
문구 구성: 제목 → 광고 표시("광고 · 제휴 링크 포함 | Ad · affiliate links", FTC 규정상 첫 125자 안에 필수)
→ 한국어 소개 → 영어 소개 → sift41.com 안내 → 분류별 해시태그 30개.
문구에 아마존 링크는 넣지 않는다 (사이트로 유도). 2,200자 초과 시 영어 소개를 뺀다.
...
- 아마존: 사이트 푸터에 필수 문구("As an Amazon Associate we earn from qualifying purchases.") 추가됨 (2026-08-08).
  인스타 계정을 아마존 어소시에이트 계정의 사이트 목록에 등록할지는 Paula 판단 대기 —
  등록 전까지 인스타 문구에 아마존 링크를 넣지 않는 현재 방식이 안전하다.
